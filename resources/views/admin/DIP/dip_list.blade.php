...
<header class="content__title">
                ...
                <div class="card">
                    <div class="card-body">
                    <h2  class="card-title"  style="font-size:30px;text-align:center">Program / DIP List Form</h2>
                    <a class="btn btn-outline-success" style="font-size:18px;background:auto;box-shadow:2px 2px 20px auto;" href="{{route('DIP.create')}}">New Entry</a>
                        <div class="table-responsive">
                            <table id="data-table" class="table">
                                ...
                                <td>{{$DIP->monthly_report}}</td>
                                <td>{{$DIP->status_achieved}}</td>
                                <td>{{$DIP->status_not_achieved}}</td>
                                 <td>
                                     <a href="{{route('DIP.show',$DIP->id)}}"><i style="color:#0043ff;font-size:18px;padding-left:25px;" class="zmdi zmdi-info zmdi-hc-fw"></i></a>
                                 </td>
                                 <td>
                                     <a href="{{route('DIP.edit',$DIP->id)}}"><i style="font-size:18px;color:green;padding-left:10px;" class="zmdi zmdi-edit zmdi-hc-fw"></i></a>
                                 </td>
                                 
                                 <td>
                                        <form  method="post" id="form_delete{{$loop->index}}" style="diplay:none;" action="{{route('DIP.destroy',$DIP->id)}}">
                                        {{csrf_field()}}
                                        {{method_field('delete')}}
                                        </form>
